      ...
      <div class="container">
          <div class="menu-container">
            ...
            <div class=" proj-contain menu">
             ...
             <div
             style="text-align: center; padding: 5px;padding-top: 15px;border-top: 1px solid gray;margin-bottom: 0px;text-decoration: underline; font-weight: bolder;font-size: 14px;">
             Проекты
             </div>
            ...
            <div v-else ><button style="margin-top:20px;" class="addButton" @click="add = true">Создать проект</button> </div>
          </div>
        <div style="margin-bottom:20px;" class="task-container" >
          <div class="title"><h1>{{title}}</h1></div>
          <div style="text-align: center;margin: 25px 0;color: #4f4f4f;font-style: italic;" v-if="!task_length">
      ...
      export default {
        ...
        methods: {
          ...
          getTasks(args){
            ...
            console.log(args.id)
            $.ajax({
              url: `http://127.0.0.1:8000/tasks/?project=${args.id}&date_ch=&status=False`,
              type: 'GET',
              success: (response) => {
                console.log(response)
                if(response.length !== 0)
                  this.task_length = true
                else
                  this.task_length = false
          ...
          getProjects() {
            $.ajax({
              ...
              type: 'GET',
              success: (response) => {
              this.projects_length = response.length
              console.log(this.projects_length)
          ...
          getToday() {
            ...
            console.log(date)
            $.ajax({
              url: `http://127.0.0.1:8000/tasks/?project=&date_ch=${date}&status=False`,
              type: 'GET',
              success: (response) => {
                  if(response.length !== 0)
                    this.task_length = true
          ...
          getArchive(){
            ...
            this.tasks = []
            $.ajax({
              url: `http://127.0.0.1:8000/tasks/?project=&date_ch=&status=True`,
              type: 'GET',
              success: (response) => {
                if(response.length !== 0)
                   this.task_length = true
